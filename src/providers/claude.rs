...
mod tests {
    ...
    fn skips_command_bookkeeping_messages() {
        for cmd in &["/exit", "/resume", "/clear", "/compact", "/mcp", "/config", "/help"] {
            let content = format!("<command-name>{cmd}</command-name><command-message>{cmd}</command-message>");
            let value = json!({ "isMeta": false, "message": { "role": "user", "content": content } });
            assert!(should_skip_message(&value, &format!("<command-name>{cmd}</command-name><command-message>{cmd}</command-message>")), "should skip /{cmd}");
        }
    }
}
